...
## إصلاحات الموقع والتوصيل — 2026-08-25

أضيفت نافذة تحديد GPS حقيقية إلى تسجيل التاجر والموصل مع الإبقاء على إدخال الولاية والبلدية والعنوان، وتُحفظ الإحداثيات في `profiles` و`merchants` و`couriers` بعد التحقق بالبريد. لا تُنشأ إحداثيات افتراضية ولا تُستبدل إحداثيات المستخدم ببيانات تجريبية.

تم توحيد حفظ اختيار مزود التوصيل للتاجر عبر `has_own_delivery`. عند اختيار توصيل المنصة لا يعتمد مسار الطلب على GPS الخاص بالمحل، كما أن فشل الحصول على GPS لا ينبغي أن يمنع الاستلام الذاتي أو تأكيد الطلب إذا كانت بيانات العنوان المطلوبة متوفرة.

أضيف ترحيل `20260906_profile_location_blacklist_guard.sql` الذي يعيد تعريف `public.is_customer_blacklisted(uuid)` بصلاحية `security definer` ومسار بحث مضبوط، مع منع التنفيذ العام وقصره على `authenticated`. الترحيل قابل لإعادة التشغيل ولم يُدخل أو يعدّل أو يحذف أي صف.

أضيفت حواجز Vitest مستقلة لهذه المسارات. النتيجة الحالية: **13 ملف اختبار، 78 اختباراً ناجحاً**، كما نجح بناء الويب الإنتاجي. يبقى التحقق الميداني على Android ضرورياً للتأكد من أذونات GPS وسلوك النظام في الجهاز الفعلي.
